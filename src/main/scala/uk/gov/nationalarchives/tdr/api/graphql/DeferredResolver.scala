...
class DeferredResolver extends sangria.execution.deferred.DeferredResolver[ConsignmentApiContext] {
  ...
  override def resolve(deferred: Vector[Deferred[Any]], context: ConsignmentApiContext, queryState: Any)(implicit ec: ExecutionContext): Vector[Future[Any]] = {
    deferred.map {
      ...
      case DeferFiles(consignmentId, paginationInput: Option[PaginationInput], queriedFileFields) =>
...
case class DeferFiles(consignmentId: UUID, paginationInput: Option[PaginationInput] = None, queriedFileFields: QueriedFileFields) extends Deferred[List[File]]
