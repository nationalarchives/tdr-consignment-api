...
trait GraphQLServerBase {
  ...
  protected def generateConsignmentApiContext(accessToken: Token, db: JdbcBackend#Database)(implicit ec: ExecutionContext): ConsignmentApiContext = {
    ...
    val finalTransferConfirmationService = new FinalTransferConfirmationService(new ConsignmentMetadataRepository(db), consignmentStatusRepository, metadataReviewLogRepository, uuidSource, timeSource)
