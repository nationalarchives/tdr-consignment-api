...
import uk.gov.nationalarchives.tdr.api.graphql.fields.FileFields.{AddFileAndMetadataInput, AllDescendantsInput, FileCustom, FileMatches}
...
class FileService(fileRepository: FileRepository,
  ...
  def getFileHierarchy(fileId: UUID): Future[Seq[FileCustom]] = {
    fileRepository.getFileHierarchy(fileId)
  }
