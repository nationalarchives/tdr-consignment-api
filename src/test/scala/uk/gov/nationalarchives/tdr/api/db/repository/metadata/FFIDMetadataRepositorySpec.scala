...
class FFIDMetadataRepositorySpec extends TestContainerUtils with ScalaFutures with Matchers {
  ...
  "countProcessedFfidMetadata" should "return the number of FFIDMetadata for files in a specified consignment" in withContainers {
    case container: PostgreSQLContainer =>
      ...
      utils.createConsignment(consignmentOne, userId, consignmentRef = "TEST-1")
      utils.createConsignment(consignmentTwo, userId, consignmentRef = "TEST-2")
  ...
  "getFFIDMetadata" should "return files with ffidMetadata and ffidMetadataMatches that match the metadata applied to them" in withContainers {
    case container: PostgreSQLContainer =>
      ...
      val fileOneFfidMetadataId = utils.addFFIDMetadata(fileOneId, fileOneSoftware, fileOneSoftwareVersion,
