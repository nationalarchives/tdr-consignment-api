...
class ValidateFileMetadataServiceSpec extends AnyFlatSpec with MockitoSugar with Matchers with ScalaFutures {
  ...
  val validation: TableFor2[String, Boolean] = Table(
    ("name", "Value"),
    ("with validation library", false),
    ("without validation library", true)
  )

  forAll(validation) { (name, value) =>
    {
      s"validateAdditionalMetadata $name" should "not update 'additional metadata' statuses if properties to validate are not 'additional metadata' properties" in {
        val testSetUp = new ValidatePropertiesSetUp(value)
      ...
      s"validateAdditionalMetadata $name" should "update 'additional metadata' statuses to 'Completed' for multiple files where there are no missing dependencies" in {
        val testSetUp = new ValidatePropertiesSetUp(value)
      ...
      s"validateAdditionalMetadata $name" should "update 'ClosureMetadata' status to 'Incomplete' for multiple files where there are missing dependencies" in {
        val testSetUp = new ValidatePropertiesSetUp(value)
        val userId = testSetUp.userId
        val fileId1 = testSetUp.fileId1
        val fileId2 = testSetUp.fileId2
      ...
      s"validateAdditionalMetadata $name" should "update 'additional metadata' statuses to 'NotEntered' for multiple files where all existing property values match defaults" in {
        val testSetUp = new ValidatePropertiesSetUp(value)
        val userId = testSetUp.userId
        val fileId1 = testSetUp.fileId1
        val fileId2 = testSetUp.fileId2
      ...
      s"validateAdditionalMetadata $name" should "update 'additional metadata' statuses to 'NotEntered' for multiple files where there are no existing additional metadata properties" in {
        val testSetUp = new ValidatePropertiesSetUp(value)
  ...
  private class ValidatePropertiesSetUp(val blockValidationLibrary: Boolean = true) {
    ...
    when(mockConfig.getBoolean(any[String])).thenReturn(blockValidationLibrary)
    val service = new ValidateFileMetadataService(mockCustomMetadataService, mockDisplayPropertiesService, mockFileMetadataRepository, mockFileStatusRepository, mockConfig)
    val mockFields: Seq[CustomMetadataField] = mockCustomMetadataFields()
