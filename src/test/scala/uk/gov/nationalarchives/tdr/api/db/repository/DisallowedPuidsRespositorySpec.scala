...
class DisallowedPuidsRespositorySpec extends TestContainerUtils with ScalaFutures with Matchers {
  ...
  "getDisallowedPuid" should "return disallowed puid if the puid exists" in withContainers {
  ...
  "getDisallowedPuid" should "return 'None' if puid not present" in withContainers {
    case container: PostgreSQLContainer =>
      val db = container.database
      val utils = TestUtils(db)
      val disallowedPuidsRepository = new DisallowedPuidsRepository(db)
  ...
  "activeReasons" should "return all disallowed reasons that are active" in withContainers {
    case container: PostgreSQLContainer =>
      val db = container.database
      val utils = TestUtils(db)
      val disallowedPuidsRepository = new DisallowedPuidsRepository(db)
