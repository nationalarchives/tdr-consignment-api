...
class FileRouteSpec extends TestContainerUtils with Matchers with TestRequest {
  ...
  "allDescendants" should "return parent and all descendants of the given single parent id" in withContainers {
    case container: PostgreSQLContainer =>
      val utils = TestUtils(container.database)
      createConsignmentStructure(utils)
  ...
  "allDescendants" should "return an empty response if no parent ids passed" in withContainers {
    case container: PostgreSQLContainer =>
      val utils = TestUtils(container.database)
      createConsignmentStructure(utils)
  ...
  "allDescendants" should "not allow a user to get descendants for a consignment that they did not create" in withContainers {
    case container: PostgreSQLContainer =>
      val utils = TestUtils(container.database)
  ...
  "allDescendants" should "return an error where no consignment id input argument provided" in withContainers {
    case container: PostgreSQLContainer =>
      val utils = TestUtils(container.database)
      createConsignmentStructure(utils)
