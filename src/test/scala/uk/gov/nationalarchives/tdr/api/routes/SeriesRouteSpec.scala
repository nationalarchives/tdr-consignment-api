...
class SeriesRouteSpec extends TestContainerUtils with Matchers with TestRequest {
  ...
  val runTestQuery: (String, String) => GraphqlQueryData = runTestRequest[GraphqlQueryData](getSeriesJsonFilePrefix)
  val runTestMutation: (String, String) => GraphqlMutationData = runTestRequest[GraphqlMutationData](addSeriesJsonFilePrefix)
