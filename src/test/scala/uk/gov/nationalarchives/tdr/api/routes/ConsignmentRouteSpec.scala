...
class ConsignmentRouteSpec extends AnyFlatSpec with Matchers with TestRequest with TestContainerForEach {
  ...
  override val containerDef: ContainerDef = PostgreSQLContainer.Def(
    dockerImageName = DockerImageName.parse(s"${sys.env("MANAGEMENT_ACCOUNT").trim}.dkr.ecr.eu-west-2.amazonaws.com/consignment-api-data").asCompatibleSubstituteFor("postgres"),
    databaseName = "consignmentapi",
    username = "tdr",
    password = "password"
  )

  def config(container: PostgreSQLContainer) = {
    ConfigFactory.load()
...
//    checkConsignmentExists(response.data.get.addConsignment.consignmentid.get, utils)
  ...
  "addConsignment" should "create a consignment of type 'judgment' when judgment consignment type provided and the user is a judgment user" in withContainers { case container: PostgreSQLContainer =>
    val expectedResponse: GraphqlMutationData = expectedMutationResponse("data_judgment_consignment_type")
    val response: GraphqlMutationData = runTestMutation("mutation_judgment_consignment_type", validJudgmentUserToken(body = "default-transferring-body-code"), config(container))
    response.data.get.addConsignment should equal(expectedResponse.data.get.addConsignment)
  ...
  "addConsignment" should "throw an error if no series id, the user is a judgment user but the consignment type is not 'judgment'" in withContainers { case container: PostgreSQLContainer =>
  ...
  "addConsignment" should "throw an error if 'standard' user's body is not the same as the series body" in withContainers { case container: PostgreSQLContainer =>
  ...
  "addConsignment" should "throw an error if an invalid consignment type is provided" in withContainers { case container: PostgreSQLContainer =>
    ...
    val response: GraphqlMutationData = runTestMutation("mutation_invalid_consignment_type", validUserToken(body = "default-transferring-body-code"), config(container))
    response.errors.head.message should equal(expectedResponse.errors.head.message)
  }
  ...
  "addConsignment" should "not allow a user to link a consignment to a series from another transferring body" in withContainers { case container: PostgreSQLContainer =>
    databaseUtils(container).addSeries(
      fixedSeriesId,
      fixedBodyId,
      "Mock series")
