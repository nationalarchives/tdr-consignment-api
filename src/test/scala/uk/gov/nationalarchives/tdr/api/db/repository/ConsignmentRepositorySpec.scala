...
import uk.gov.nationalarchives.tdr.api.graphql.fields.ConsignmentFields.{ConsignmentFilters, StartUploadInput, UpdateClientSideDraftMetadataFileNameInput, UpdateExportDataInput, UpdateMetadataSchemaLibraryVersionInput}
...
class ConsignmentRepositorySpec extends TestContainerUtils with ScalaFutures with Matchers {
  ...
  "updateClientSideDraftMetadataFileName" should "update the draft metadata file name for the consignment" in withContainers {
    case container: PostgreSQLContainer =>
      val consignmentId = UUID.fromString("a3088f8a-59a3-4ab3-9e50-1677648e8186")
      val db = container.database
      val consignmentRepository = new ConsignmentRepository(db, new CurrentTimeSource)
