...
trait TestRequest extends AnyFlatSpec with Matchers {
  ...
  def runTestRequest[A](prefix: String, db: Option[JdbcBackend#DatabaseDef] = None)(queryFileName: String, token: String)(implicit decoder: Decoder[A], classTag: ClassTag[A]): A = {
    ...
    val response: Response[IO] = Http4sServer(db.get).corsWrapper
      .run(
