package uk.gov.nationalarchives.tdr.api.routes.consignment
...
class ConsignmentStatusRouteSpec extends TestContainerUtils with Matchers with TestRequest {
  ...
  "setUploadConsignmentStatusValueToComplete" should "update consignment status" in withContainers {
    case container: PostgreSQLContainer =>
      ...
      val response = runTestRequest[GraphqlMutationData](markUploadAsCompletedJsonFilePrefix)("mutation_data_all", token, config(container))
  ...
  "markUploadAsCompleted" should "not allow a user to update the consignment status of a consignment that they did not create" in withContainers {
    case container: PostgreSQLContainer =>
      ...
      val response = runTestRequest[GraphqlMutationData](markUploadAsCompletedJsonFilePrefix)("mutation_not_owner", token, config(container))
  ...
  "markUploadAsCompleted" should "return an error if a consignment that doesn't exist is queried" in withContainers {
    case container: PostgreSQLContainer =>
      ...
      val response = runTestRequest[GraphqlMutationData](markUploadAsCompletedJsonFilePrefix)("mutation_invalid_consignmentid", token, config(container))
