...
class FileMetadataRepositorySpec extends TestContainerUtils with ScalaFutures with Matchers {
  ...
  "countProcessedChecksumInConsignment" should "return 0 if a consignment has no files" in withContainers { case container: PostgreSQLContainer =>
  ...
  "countProcessedChecksumInConsignment" should "return 0 if consignment has no checksum metadata for files" in withContainers { case container: PostgreSQLContainer =>
    val db = container.database
    val utils = TestUtils(db)
    val fileMetadataRepository = new FileMetadataRepository(db)
  ...
  "countProcessedChecksumInConsignment" should "return the number of fileMetadata for files in a specified consignment" in withContainers { case container: PostgreSQLContainer =>
    val db = container.database
    val utils = TestUtils(db)
    val fileMetadataRepository = new FileMetadataRepository(db)
  ...
  "countProcessedChecksumInConsignment" should "return number of fileMetadata rows with repetitive data filtered out" in withContainers { case container: PostgreSQLContainer =>
    val db = container.database
    val utils = TestUtils(db)
    val fileMetadataRepository = new FileMetadataRepository(db)
  ...
  "updateFileMetadataProperties" should "update the value and userId for the correct metadata rows and return the number of rows it updated" in withContainers {
    case container: PostgreSQLContainer =>
      ...
      utils.createConsignment(consignmentId, userId)
      List(fileIdOne, fileIdTwo, fileIdThree).foreach(fileId => utils.createFile(fileId, consignmentId))
      utils.addFileProperty("FilePropertyOne")
      utils.addFileProperty("FilePropertyTwo")
