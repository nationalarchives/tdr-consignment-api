...
class ConsignmentRepositorySpec extends TestContainerUtils with Matchers with ScalaFutures {
  ...
  "getSeriesOfConsignment" should "get the series for a consignment" in withContainers {
    case container: PostgreSQLContainer =>
      ...
      utils.addSeries(seriesId, fixedBodyId, seriesCode)
      utils.createConsignment(consignmentId, userId)
  ...
  "getNextConsignmentSequence" should "get the next sequence ID number for a consignment row" in withContainers {
    case container: PostgreSQLContainer =>
      ...
      utils.createConsignment(consignmentIdOne, userId, consignmentRef = "TEST-CON1")
      utils.createConsignment(consignmentIdTwo, userId, consignmentRef = "TEST-CON2")
  ...
  private def createConsignments(utils: DatabaseUtils): Unit = {
