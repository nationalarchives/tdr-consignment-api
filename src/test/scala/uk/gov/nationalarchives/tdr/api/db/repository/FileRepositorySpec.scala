...
class FileRepositorySpec extends TestContainerUtils with ScalaFutures with Matchers {
  ...
  "countFilesInConsignment" should "return the total number of files and folders in a consignment given no file type filter" in withContainers {
    case container: PostgreSQLContainer =>
      val db = container.database
      val utils = TestUtils(db)
      val fileRepository = new FileRepository(db)
      ...
      val consignmentFiles = fileRepository.countFilesInConsignment(consignmentId,
        fileTypeIdentifier = None).futureValue

      consignmentFiles shouldBe 4
  }

  "countFilesInConsignment" should "return the total number of files in a consignment given a fileId filter" in withContainers {
    case container: PostgreSQLContainer =>
      val db = container.database
      val utils = TestUtils(db)
      val fileRepository = new FileRepository(db)
      ...
      val consignmentFiles = fileRepository.countFilesInConsignment(consignmentId,
        parentId = Some(folderId)).futureValue
