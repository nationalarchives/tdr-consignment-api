...
class FileRepositorySpec extends TestContainerUtils with ScalaFutures with Matchers with TableDrivenPropertyChecks {
  ...
  "countProcessedAvMetadataInConsignment" should "return 0 if consignment has no AVmetadata for files" in withContainers { case container: PostgreSQLContainer =>
    val db = container.database
    val utils = TestUtils(db)
    val fileRepository = new FileRepository(db)
  ...
  "countProcessedAvMetadataInConsignment" should "return the number of AVmetadata for files in a specified consignment" in withContainers { case container: PostgreSQLContainer =>
    val db = container.database
    val utils = TestUtils(db)
    val fileRepository = new FileRepository(db)
  ...
  "countProcessedAvMetadataInConsignment" should "return number of AVmetadata rows with repetitive data filtered out" in withContainers { case container: PostgreSQLContainer =>
    val db = container.database
    val utils = TestUtils(db)
    val fileRepository = new FileRepository(db)
  ...
  forAll(redactedFilesTable) { (redactedFileName, originalFileName) =>
    "getRedactedFilePairs" should s"return a file with $originalFileName for redacted file $redactedFileName" in withContainers { case container: PostgreSQLContainer =>
      val db = container.database
      val utils = TestUtils(db)
  ...
  forAll(invalidRedactedFormat) { (redactedFileName, originalFileName) =>
    "getRedactedFilePairs" should s"return an empty list for $redactedFileName and $originalFileName" in withContainers { case container: PostgreSQLContainer =>
      val db = container.database
      val utils = TestUtils(db)
      val consignmentId = UUID.randomUUID()
      val parentId = UUID.randomUUID()
  ...
  "getRedactedFilePairs" should "return the same original file ID for two valid redacted files" in withContainers { case container: PostgreSQLContainer =>
    val db = container.database
    val utils = TestUtils(db)
    val consignmentId = UUID.randomUUID()
    val parentId = UUID.randomUUID()
  ...
  "getRedactedFilePairs" should "return a single entry when two are available if filters are set" in withContainers { case container: PostgreSQLContainer =>
    val db = container.database
    val utils = TestUtils(db)
    val consignmentId = UUID.randomUUID()
    val parentId = UUID.randomUUID()
