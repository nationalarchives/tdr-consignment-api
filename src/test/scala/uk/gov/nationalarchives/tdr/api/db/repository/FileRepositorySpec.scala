...
class FileRepositorySpec extends TestContainerUtils with ScalaFutures with Matchers {
  ...
  "getFiles" should "return files and file metadata where 'selectedFileIds' filter applied" in withContainers {
    case container: PostgreSQLContainer =>
      ...
      val files = fileRepository.getFiles(consignmentId, FileFilters(selectedFileIds = Some(List(fileOneId)))).futureValue
      files.forall(_._1.fileid == fileOneId) shouldBe true
  }

  "getFiles" should "return files and file metadata where and 'file' type and 'selectedFileIds' filter applied" in withContainers {
    case container: PostgreSQLContainer =>
      val db = container.database
      val utils = TestUtils(db)
      val fileRepository = new FileRepository(db)
