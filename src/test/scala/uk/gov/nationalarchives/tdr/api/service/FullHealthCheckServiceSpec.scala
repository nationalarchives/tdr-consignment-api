...
class FullHealthCheckServiceSpec extends TestContainerUtils with ScalaFutures with Matchers {
  ...
  "checkDbIsUpAndRunning" should "throw an exception if db has no Transferring Bodies in it" in withContainers { case containers: PostgreSQLContainer =>
  ...
  "checkDbIsUpAndRunning" should "return Unit if db has 1 or more Transferring Bodies in it" in withContainers { case container: PostgreSQLContainer =>
  ...
  "checkDbIsUpAndRunning" should "throw an exception if the database is unavailable" in withContainers { case container: PostgreSQLContainer =>
